...
const v55Markers = [
  'APP_VERSION_LABEL','v5.6','brand-logo','aerizen-emblem-512.png','function InvoiceForm','Identitas Invoice',
...
for (const marker of v55Markers) {
  if (!app.includes(marker) && !css.includes(marker) && !preview.includes(marker)) {
    console.error(`Missing v5.6 retained/UI marker: ${marker}`);
    process.exit(1);
  }
}

for (const marker of ['asset-summary-grid','asset-summary-card','asset-summary-value','v5.6 GitHub build fix']) {
  if (!app.includes(marker) && !css.includes(marker)) {
    console.error(`Missing v5.6 GitHub build fix marker: ${marker}`);
    process.exit(1);
  }
}

const previewMarkers = [
  'Aerizen Asset Management v5.6','Asset Management System','Identitas Invoice','Upload Faktur Pajak',
...
console.log('Aerizen v5.6 smoke-test passed: all prior features retained and Aset 360 UI remains responsive and tidy.');
